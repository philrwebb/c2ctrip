...
This is an LLM-maintained knowledge base on trekking the Coast to Coast walk in the UK. The LLM writes and maintains all files under `wiki/`. The human curates raw sources and directs queries. The human never edits wiki files directly.
...
- **Walk Segments**: `short`, `medium`, `long`
- **Stops**: `Towns`, `villages`, `farm`
